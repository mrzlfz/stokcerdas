...
export class BenchmarkingService {
  ...
  async generateBenchmarkingAnalysis(
    ...
    try {
      this.logger.debug(`Generating benchmarking analysis for tenant ${tenantId}`);

      // Calculate current metrics for the tenant
      const currentMetrics = await this.calculateCurrentMetrics(tenantId, query);
      ...
      const metricsToAnalyze = query.metrics || [MetricType.REVENUE, MetricType.MARGIN, MetricType.TURNOVER];
      ...
      for (const metric of metricsToAnalyze) {
        ...
        if (benchmark) {
          const comparison = this.generateComparison(metric, currentValue, benchmark, query);
          data.push(comparison);
        }
      }

      // Calculate summary metrics
      const summary = this.calculateBenchmarkingSummary(data, query.benchmarkType);

      // Generate peer comparison if requested
      let peerComparison;
      if (query.includePeerComparison) {
        peerComparison = await this.generatePeerComparison(tenantId, currentMetrics, query);
      }
      ...
      throw new BadRequestException(`Failed to generate benchmarking analysis: ${error.message}`);
    }
  }
  ...
  private async calculateCurrentMetrics(
    ...
    const profitResult = await this.transactionRepository
      ...
      .select('SUM(transaction.quantity * (transaction.unitCost - product.costPrice))', 'totalProfit')
      .getRawOne();
    ...
    if (metrics[MetricType.REVENUE] > 0) {
      metrics[MetricType.MARGIN] = (metrics[MetricType.PROFIT] / metrics[MetricType.REVENUE]) * 100;
    }
    ...
    const avgInventoryValue = Number(avgInventoryResult?.avgInventoryValue) || 0;
    ...
    const growthMetrics = await this.calculateGrowthMetrics(tenantId, startDate, endDate);
  ...
  private generateComparison(
    ...
    const relativePerformance = benchmarkValue > 0 ? (currentValue / benchmarkValue) * 100 : 0;
    ...
    if (benchmark.percentile90 && benchmark.percentile75 && benchmark.percentile50 && benchmark.percentile25) {
      ...
      else if (relativePerformance >= 110) performanceCategory = 'above_average';
    ...
    const recommendations = this.generateMetricRecommendations(metric, currentValue, benchmarkValue, performanceCategory);
  ...
  private calculateBenchmarkingSummary(
    ...
    const overallScore = data.reduce((sum, item) => sum + item.relativePerformance, 0) / data.length;
    const metricsAboveBenchmark = data.filter(item => item.relativePerformance > 100).length;
    const metricsBelowBenchmark = data.filter(item => item.relativePerformance < 100).length;

    const topPerformingMetrics = data
      .filter(item => item.performanceCategory === 'excellent' || item.performanceCategory === 'above_average')
      .map(item => item.metricName);

    const improvementAreas = data
      .filter(item => item.performanceCategory === 'below_average' || item.performanceCategory === 'poor')
      .map(item => item.metricName);
  ...
  private async generatePeerComparison(
    ...
    const sortedPeers = mockPeerData.sort((a, b) => b.metrics[MetricType.REVENUE] - a.metrics[MetricType.REVENUE]);
    const yourRanking = sortedPeers.findIndex(peer => yourRevenue > peer.metrics[MetricType.REVENUE]) + 1;
    
    // Calculate industry average
    const industryAverage = sortedPeers.reduce((sum, peer) => sum + peer.metrics[MetricType.REVENUE], 0) / sortedPeers.length;
  ...
  private getIndustryStandardBenchmarks(): Record<MetricType, any> {
    ...
    return {
      ...
      [MetricType.GROWTH]: { value: 12.5, percentile50: 12.5, percentile75: 18.2, percentile90: 25.0 },
      [MetricType.REVENUE]: { value: 750000000, percentile50: 750000000, percentile75: 1200000000, percentile90: 2000000000 },
      [MetricType.PROFIT]: { value: 191250000, percentile50: 191250000, percentile75: 306000000, percentile90: 510000000 },
      [MetricType.VOLUME]: { value: 8500, percentile50: 8500, percentile75: 12000, percentile90: 18000 },
    };
  }
  ...
  private getHistoricalAverageBenchmarks(currentMetrics: Record<MetricType, number>): Record<MetricType, any> {
    ...
    return {
      ...
      [MetricType.TURNOVER]: { value: currentMetrics[MetricType.TURNOVER] * 1.1 },
      [MetricType.GROWTH]: { value: 8.5 }, // Historical average growth
      [MetricType.REVENUE]: { value: currentMetrics[MetricType.REVENUE] * 0.88 },
  ...
  private async calculateGrowthMetrics(tenantId: string, currentStartDate: Date, currentEndDate: Date): Promise<any> {
  ...
  private generateMockPeerData(): PeerData[] {
    ...
    const baseMetrics = [300000000, 500000000, 750000000, 1200000000, 2000000000]; // Revenue ranges

    for (let i = 0; i < 50; i++) {
      const revenue = baseMetrics[Math.floor(Math.random() * baseMetrics.length)] * (0.8 + Math.random() * 0.4);
      ...
      peers.push({
        ...
        category: 'retail',
        size: revenue > 1000000000 ? 'large' : revenue > 500000000 ? 'medium' : 'small',
        region: 'indonesia',
      });
    }
  ...
  private generateMetricRecommendations(
    ...
    switch (metric) {
      case MetricType.MARGIN:
        if (performanceCategory === 'below_average' || performanceCategory === 'poor') {
          recommendations.push('Review pricing strategy to improve profit margins');
          recommendations.push('Analyze supplier costs and negotiate better terms');
          recommendations.push('Focus on high-margin products in marketing efforts');
        }
        break;
      case MetricType.TURNOVER:
        if (performanceCategory === 'below_average' || performanceCategory === 'poor') {
          recommendations.push('Improve inventory management to reduce holding costs');
          recommendations.push('Implement demand forecasting to optimize stock levels');
      ...
      case MetricType.REVENUE:
        if (performanceCategory === 'below_average' || performanceCategory === 'poor') {
          ...
          recommendations.push('Expand product range or target new customer segments');
      ...
      case MetricType.GROWTH:
        if (currentValue < benchmarkValue) {
          ...
          recommendations.push('Invest in digital marketing and online presence');
          recommendations.push('Consider expanding to new locations or channels');
        }
        break;
    }

    if (recommendations.length === 0) {
      recommendations.push('Continue current strategies to maintain performance');
    }
