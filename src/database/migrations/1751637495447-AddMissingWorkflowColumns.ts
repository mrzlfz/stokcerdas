...
export class AddMissingWorkflowColumns1751637495447
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const hasTriggerConfig = await queryRunner.hasColumn(
      'workflows',
      'triggerConfig',
    );
    if (!hasTriggerConfig) {
    ...
    const hasWorkflowConfig = await queryRunner.hasColumn(
      'workflows',
      'workflowConfig',
    );
    ...
    const hasNotificationConfig = await queryRunner.hasColumn(
      'workflows',
      'notificationConfig',
    );
    ...
    const hasIsTemplate = await queryRunner.hasColumn(
      'workflows',
      'isTemplate',
    );
    if (!hasIsTemplate) {
    ...
    const hasPausedUntil = await queryRunner.hasColumn(
      'workflows',
      'pausedUntil',
    );
    if (!hasPausedUntil) {
    ...
    const hasPauseReason = await queryRunner.hasColumn(
      'workflows',
      'pauseReason',
    );
    if (!hasPauseReason) {
    ...
    const hasPreviousVersionId = await queryRunner.hasColumn(
      'workflows',
      'previousVersionId',
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Drop indexes
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_workflows_consecutive_failures"`,
    );
    await queryRunner.query(
    ...
    // Drop columns
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "estimatedCostPerExecution"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "totalDataProcessed"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "totalStepsExecuted"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "totalProcessingTime"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "consecutiveFailures"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "lastErrorAt"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "lastErrorMessage"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "lastExecutionDuration"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "nextExecutionAt"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "successRate"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "averageExecutionTime"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "cancelledExecutions"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "timeoutExecutions"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "failedExecutions"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "successfulExecutions"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "totalExecutions"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "permissions"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "ownerId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "templateId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "previousVersionId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "version"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "pauseReason"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "pausedUntil"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "isPaused"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "isTemplate"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "isActive"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "metadata"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "tags"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "variables"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "notificationConfig"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "workflowConfig"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "triggerConfig"`,
    );
    await queryRunner.query(
      `ALTER TABLE "workflows" DROP COLUMN IF EXISTS "priority"`,
    );
