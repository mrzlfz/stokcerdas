...
import { PurchaseOrder, PurchaseOrderType, PurchaseOrderPriority } from '../../purchase-orders/entities/purchase-order.entity';
import { CreatePurchaseOrderDto } from '../../purchase-orders/dto/create-purchase-order.dto';

import { ReorderCalculationService, ReorderCalculationResult } from './reorder-calculation.service';
import { SupplierSelectionService, SupplierSelectionResult } from './supplier-selection.service';
...
export class AutomatedPurchasingService {
   ...
   */
  async executeAutomatedPurchase(request: AutomatedPurchaseRequest): Promise<AutomatedPurchaseResult> {
    const startTime = Date.now();
    
    try {
      this.logger.log(`Starting automated purchase execution for tenant ${request.tenantId}`);
      ...
      const { inventoryItem, product } = await this.getInventoryAndProduct(reorderRule);
      ...
      try {
        ...
        const reorderCalculation = await this.reorderCalculationService.calculateReorderRecommendation({
          reorderRule,
          inventoryItem,
          product,
        ...
        if (!reorderCalculation.isValid) {
          throw new Error(`Reorder calculation failed: ${reorderCalculation.validationErrors.join(', ')}`);
        }

        // Check if reorder is needed
        if (!reorderCalculation.shouldReorderNow && !request.forceExecution) {
          return this.createSkippedResult(execution, reorderCalculation, 'Reorder not needed at this time');
        }

        // Step 2: Select supplier
        const supplierSelection = await this.supplierSelectionService.selectSupplier({
          product,
          reorderRule,
          orderQuantity: request.overrides?.orderQuantity || reorderCalculation.recommendedOrderQuantity,
          urgencyLevel: request.overrides?.urgencyLevel || reorderCalculation.urgencyScore,
          budgetConstraint: reorderRule.remainingBudget < Infinity ? reorderRule.remainingBudget : undefined,
          deliveryDeadline: this.calculateDeliveryDeadline(reorderCalculation),
          excludeSuppliers: [],
          includeOnlySuppliers: request.overrides?.selectedSupplierId ? [request.overrides.selectedSupplierId] : undefined,
        });
        ...
        const shouldCreatePO = this.shouldCreatePurchaseOrder(reorderCalculation, supplierSelection, reorderRule);
        ...
        if (shouldCreatePO && !request.dryRun) {
          ...
          await this.handleApprovalWorkflow(purchaseOrder, reorderRule, reorderCalculation);
        }

        // Step 6: Update execution record
        const finalQuantity = request.overrides?.orderQuantity || reorderCalculation.recommendedOrderQuantity;
        ...
        await this.updateExecutionRecord(execution, {
          ...
          calculationDetails: {
            ...
            leadTimeDemand: reorderCalculation.safetyStockCalculation.leadTimeDemand,
            ...
            eoqCalculation: reorderCalculation.eoqCalculation?.economicOrderQuantity,
            seasonalFactor: reorderCalculation.demandAnalysis.seasonalityFactor,
            supplierScores: { [supplierSelection.selectedSupplier.supplierId]: supplierSelection.selectedSupplier.totalScore },
          },
        });
        ...
        await this.sendNotifications(reorderRule, reorderCalculation, supplierSelection, purchaseOrder);

        // Step 9: Emit events
        this.emitAutomationEvents(reorderRule, purchaseOrder, reorderCalculation);
        ...
        return {
          ...
          isAutomaticallyApproved: this.isAutomaticallyApproved(reorderRule, finalValue),
          approvalRequired: purchaseOrder ? purchaseOrder.requiresApproval : false,
          recommendations: this.generateRecommendations(reorderCalculation, supplierSelection),
          warnings: this.generateWarnings(reorderCalculation, supplierSelection),
          errors: [],
          ...
          followUpActions: this.generateFollowUpActions(reorderCalculation, supplierSelection),
          riskLevel: this.assessRiskLevel(reorderCalculation, supplierSelection),
          riskFactors: [...reorderCalculation.riskMitigationSuggestions, ...supplierSelection.riskWarnings],
          mitigationActions: this.generateMitigationActions(reorderCalculation, supplierSelection),
        };
      ...
      return {
        ...
        mitigationActions: ['Review automation configuration', 'Manual purchase order creation may be needed'],
      };
    }
  }
   ...
   */
  async executeBulkAutomatedPurchase(request: BulkAutomatedPurchaseRequest): Promise<BulkAutomatedPurchaseResult> {
    const startTime = Date.now();
    const batchId = `batch_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    
    try {
      this.logger.log(`Starting bulk automated purchase for tenant ${request.tenantId}`);
   ...
   */
  async getEligibleReorderRulesForExecution(tenantId: string): Promise<ReorderRule[]> {
  ...
  // Helper methods
  private async getReorderRule(request: AutomatedPurchaseRequest): Promise<ReorderRule | null> {
  ...
  private async getInventoryAndProduct(reorderRule: ReorderRule): Promise<{
    ...
    const product = reorderRule.product || await this.productRepository.findOne({
  ...
  private async createExecutionRecord(reorderRule: ReorderRule): Promise<ReorderExecution> {
  ...
  private calculateDeliveryDeadline(reorderCalculation: ReorderCalculationResult): Date | undefined {
    if (reorderCalculation.daysUntilStockout < 14) {
      // Urgent delivery needed
      return new Date(Date.now() + reorderCalculation.daysUntilStockout * 24 * 60 * 60 * 1000);
    }
    return undefined;
  }
  ...
  private shouldCreatePurchaseOrder(
    ...
    if (reorderRule.remainingBudget < Infinity) {
      if (supplierSelection.selectedSupplier!.estimatedCost > reorderRule.remainingBudget) {
        return false;
      }
    }
  ...
  private buildPurchaseOrderDto(
    ...
    const quantity = overrides?.orderQuantity || reorderCalculation.recommendedOrderQuantity;
    
    return {
      supplierId: selectedSupplier.supplierId,
      type: PurchaseOrderType.STANDARD,
      priority: reorderCalculation.urgencyScore >= 8 ? PurchaseOrderPriority.URGENT : PurchaseOrderPriority.NORMAL,
      description: `Automated reorder for ${reorderRule.product.name}`,
      notes: `Generated by automation rule: ${reorderRule.name}`,
      internalNotes: `Calculation confidence: ${(reorderCalculation.confidenceLevel * 100).toFixed(1)}%, Selection confidence: ${(supplierSelection.selectionConfidence * 100).toFixed(1)}%`,
      items: [{
        productId: reorderRule.productId,
      ...
      expectedDeliveryDate: supplierSelection.predictedDeliveryDate?.toISOString(),
      requestedDeliveryDate: supplierSelection.predictedDeliveryDate?.toISOString(),
  ...
  private requiresApproval(reorderRule: ReorderRule, orderValue: number): boolean {
    if (!reorderRule.requiresApproval) return false;
    if (reorderRule.autoApprovalThreshold && orderValue <= reorderRule.autoApprovalThreshold) return false;
    return true;
  }

  private isAutomaticallyApproved(reorderRule: ReorderRule, orderValue: number): boolean {
    if (!reorderRule.isFullyAutomated) return false;
    if (reorderRule.autoApprovalThreshold && orderValue <= reorderRule.autoApprovalThreshold) return true;
    return false;
  }
  ...
  private generateTriggerReason(reorderCalculation: ReorderCalculationResult): string {
    const reasons = [];
    
    if (reorderCalculation.currentStock <= reorderCalculation.recommendedReorderPoint) {
    ...
    if (reorderCalculation.daysUntilStockout <= 7) {
      reasons.push(`Stockout predicted in ${Math.round(reorderCalculation.daysUntilStockout)} days`);
    }
  ...
  private assessRiskLevel(
    ...
    if (reorderCalculation.urgencyScore >= 9 || reorderCalculation.stockoutRisk > 0.8) {
      return 'critical';
    }
    
    if (reorderCalculation.urgencyScore >= 7 || reorderCalculation.stockoutRisk > 0.5) {
      return 'high';
    }
    
    if (reorderCalculation.urgencyScore >= 5 || reorderCalculation.confidenceLevel < 0.6) {
      return 'medium';
    }
  ...
  private async sendNotifications(
    ...
    const severity = reorderCalculation.urgencyScore >= 8 ? AlertSeverity.CRITICAL : AlertSeverity.WARNING;
    ...
    const message = purchaseOrder
      ...
      : `Reorder analysis completed for ${reorderRule.product.name}. ${reorderCalculation.shouldReorderNow ? 'Reorder recommended' : 'No action needed'}.`;
  ...
  private async getEligibleReorderRules(request: BulkAutomatedPurchaseRequest): Promise<ReorderRule[]> {
    ...
    if (request.reorderRuleIds?.length) {
      query = query.andWhere('rule.id IN (:...ids)', { ids: request.reorderRuleIds });
    }

    if (request.productIds?.length) {
      query = query.andWhere('rule.productId IN (:...productIds)', { productIds: request.productIds });
    }

    if (request.locationIds?.length) {
      query = query.andWhere('rule.locationId IN (:...locationIds)', { locationIds: request.locationIds });
    }

    return query.getMany();
  }

  private calculateBulkSummary(results: AutomatedPurchaseResult[]): BulkAutomatedPurchaseResult['summary'] {
    const successfulResults = results.filter(r => r.success && r.shouldCreatePurchaseOrder);
    
    const totalValue = successfulResults.reduce((sum, r) => sum + r.estimatedValue, 0);
    const averageOrderValue = successfulResults.length > 0 ? totalValue / successfulResults.length : 0;
    
    const uniqueSuppliers = new Set(successfulResults.map(r => r.selectedSupplierId)).size;
    const ordersRequiringApproval = successfulResults.filter(r => r.requiresApproval).length;
    const highRiskOrders = results.filter(r => r.riskLevel === 'high' || r.riskLevel === 'critical').length;
