...
export class SuppliersController {
  ...
  @ApiResponse({
    status: 409,
    description: 'Kode supplier atau email sudah ada',
  })
  async create(
  ...
  @ApiOperation({
    summary: 'Dapatkan daftar supplier dengan filter dan pagination',
  })
  @ApiResponse({ status: 200, description: 'Daftar supplier berhasil didapat' })
  async findAll(@Tenant() tenantId: string, @Query() query: SupplierQueryDto) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Statistik supplier berhasil didapat',
  })
  ...
  @ApiResponse({
    status: 400,
    description: 'Supplier masih memiliki produk aktif',
  })
  ...
  async remove(
    ...
    return {
      success: true,
      message: hardDelete
        ? 'Supplier berhasil dihapus permanent'
  ...
  async addNote(
    ...
    const supplier = await this.suppliersService.addNote(
      tenantId,
      id,
      addNoteDto,
      userId,
  ...
  @ApiResponse({
    status: 200,
    description: 'Performa supplier berhasil diupdate',
  })
  @ApiResponse({ status: 404, description: 'Supplier tidak ditemukan' })
  async updatePerformance(
    ...
    const supplier = await this.suppliersService.updatePerformance(
      tenantId,
      id,
      performanceDto,
    );
  ...
  async bulkCreate(
    ...
    const result = await this.suppliersService.bulkCreate(
      tenantId,
      bulkCreateDto,
      userId,
    );
  ...
  async bulkUpdate(
    ...
    const result = await this.suppliersService.bulkUpdate(
      tenantId,
      bulkUpdateDto,
      userId,
    );
  ...
  async bulkDelete(
    ...
    const result = await this.suppliersService.bulkDelete(
      tenantId,
      bulkDeleteDto,
      userId,
    );
  ...
  @ApiQuery({
    name: 'limit',
    required: false,
    description: 'Jumlah supplier top performer',
    type: Number,
  })
  @ApiResponse({
    status: 200,
    description: 'Laporan performa berhasil didapat',
  })
  ...
  @ApiQuery({
    name: 'days',
    required: false,
    description: 'Jumlah hari ke depan',
    type: Number,
  })
    ...
    const expiringContracts = suppliers.data
      .filter(
        supplier =>
          ...
          new Date(supplier.contractEndDate) <= endDate,
      )
      .map(supplier => ({
        id: supplier.id,
        code: supplier.code,
