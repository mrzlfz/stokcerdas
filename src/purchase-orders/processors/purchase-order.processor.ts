...
export class PurchaseOrderProcessor {
  ...
  async handleIndexPurchaseOrder(
    job: Job<{
    ...
    try {
      this.logger.log(
        `Processing index job for PO ${purchaseOrderId} with action ${action}`,
      );
      ...
      switch (action) {
        ...
        case 'softDelete':
          this.logger.log(
            `Removing PO ${purchaseOrderId} from index for ${action}`,
          );
      ...
      this.logger.log(
        `Successfully processed index job for PO ${purchaseOrderId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to process index job for PO ${purchaseOrderId}`,
        error.stack,
      );
      throw error;
    }
  ...
  async handleSendCreationNotification(
    job: Job<{
      purchaseOrderId: string;
      tenantId: string;
    }>,
    ...
    try {
      this.logger.log(
        `Sending creation notification for PO ${purchaseOrderId}`,
      );
      ...
      this.logger.log(
        `Successfully sent creation notification for PO ${purchaseOrderId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to send creation notification for PO ${purchaseOrderId}`,
        error.stack,
      );
      throw error;
    }
  ...
  async handleSendApprovalNotification(
    job: Job<{
      purchaseOrderId: string;
      tenantId: string;
    ...
    try {
      this.logger.log(
        `Sending ${action} notification for PO ${purchaseOrderId}`,
      );
      ...
      this.logger.log(
        `Successfully sent ${action} notification for PO ${purchaseOrderId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to send ${action} notification for PO ${purchaseOrderId}`,
        error.stack,
      );
      throw error;
    }
    ...
    try {
      ...
      if (generatePdf) {
        try {
          ...
          this.logger.log(
            `PDF generated for PO ${purchaseOrderId}: ${pdfPath}`,
          );
        } catch (pdfError) {
          this.logger.error(
            `Failed to generate PDF for PO ${purchaseOrderId}`,
            pdfError.stack,
          );
      ...
      this.logger.log(
        `Successfully processed send to supplier for PO ${purchaseOrderId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to process send to supplier for PO ${purchaseOrderId}`,
        error.stack,
      );
      throw error;
    }
  ...
  async handleSendStatusUpdate(
    ...
    const { purchaseOrderId, previousStatus, newStatus, reason, updatedBy } =
      job.data;

    try {
      this.logger.log(
        `Sending status update notification for PO ${purchaseOrderId}`,
      );
      ...
      this.logger.log(
        `Successfully sent status update notification for PO ${purchaseOrderId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to send status update notification for PO ${purchaseOrderId}`,
        error.stack,
      );
      throw error;
    }
  ...
  async handleGeneratePdf(
    ...
    try {
      ...
      if (emailToRecipients && emailToRecipients.length > 0) {
        ...
        this.logger.log(
          `Sending PDF for PO ${purchaseOrderId} to ${emailToRecipients.join(
            ', ',
          )}`,
        );
      }
      ...
      if (saveToStorage) {
        ...
        this.logger.log(
          `Saving PDF for PO ${purchaseOrderId} to cloud storage`,
        );
      }

      this.logger.log(
        `Successfully generated PDF for PO ${purchaseOrderId}: ${pdfPath}`,
      );
      return pdfPath;
    } catch (error) {
      this.logger.error(
  ...
  async handleBulkExport(
    ...
    try {
      this.logger.log(
        `Processing bulk export for tenant ${tenantId}, format: ${format}`,
      );
      ...
      this.logger.log(
        `Bulk export completed for tenant ${tenantId}, sending email to ${userEmail}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to process bulk export for tenant ${tenantId}`,
        error.stack,
      );
      throw error;
    }
  ...
  async handleSyncWithInventory(
    ...
    try {
      this.logger.log(
        `Syncing PO ${purchaseOrderId} with inventory for action: ${action}`,
      );
      ...
      this.logger.log(
        `Successfully synced PO ${purchaseOrderId} with inventory`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to sync PO ${purchaseOrderId} with inventory`,
        error.stack,
      );
      throw error;
    }
