...
import { BulkCreateProductDto, BulkUpdateProductDto, BulkDeleteProductDto } from '../dto/bulk-product.dto';
...
export class ProductsController {
  ...
  async create(
    ...
    const product = await this.productsService.create(tenantId, createProductDto, userId);
  ...
  @ApiOperation({ summary: 'Dapatkan daftar produk dengan filter dan pagination' })
  ...
  @ApiResponse({ status: 200, description: 'Statistik produk berhasil didapat' })
  ...
  async findByBarcode(
    ...
    const result = await this.barcodeService.findProductByBarcode(tenantId, barcode);
  ...
  async update(
    ...
    const product = await this.productsService.update(tenantId, id, updateProductDto, userId);
  ...
  async remove(
    ...
    return {
      success: true,
      message: hardDelete ? 'Produk berhasil dihapus permanent' : 'Produk berhasil dihapus',
    };
  }
  ...
  async bulkCreate(
    ...
    const result = await this.productsService.bulkCreate(tenantId, bulkCreateDto, userId);
  ...
  async bulkUpdate(
    ...
    const result = await this.productsService.bulkUpdate(tenantId, bulkUpdateDto, userId);
  ...
  async bulkDelete(
    ...
    const result = await this.productsService.bulkDelete(tenantId, bulkDeleteDto, userId);
  ...
  async generateBarcode(
    ...
    const barcode = await this.barcodeService.generateProductBarcode(tenantId, id);
  ...
  @Permissions({ resource: PermissionResource.PRODUCTS, action: PermissionAction.UPDATE })
  @ApiOperation({ summary: 'Generate barcode untuk semua produk yang belum punya' })
  @ApiQuery({ name: 'limit', required: false, description: 'Maksimal produk yang diproses', type: Number })
  @ApiResponse({ status: 200, description: 'Bulk generate barcode selesai' })
  async bulkGenerateBarcode(
    @Tenant() tenantId: string,
    @Query('limit') limit?: number,
  ) {
    const result = await this.barcodeService.generateBulkBarcodes(tenantId, limit);
    
    return {
      success: true,
      message: `Bulk generate selesai. ${result.productsUpdated + result.variantsUpdated} berhasil`,
      data: result,
    };
  }
}