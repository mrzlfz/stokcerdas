...
export class PerformanceCacheService {
  ...
  private readonly inMemoryCache = new Map<string, { value: any; expiry: number; tags: string[] }>();
  ...
  async getOrSet<T>(
    ...
    if (value === null) {
      ...
      this.logger.debug(`Cache MISS - Executed fallback: ${pattern} (${executionTime}ms)`);
    }
   ...
   */
  async invalidate(patterns: string | string[], tenantId?: string): Promise<void> {
    try {
      ...
      for (const pattern of patternsArray) {
        ...
        this.logger.debug(`Cache INVALIDATED: ${pattern}${tenantId ? ` (tenant: ${tenantId})` : ''}`);
      }
    } catch (error) {
      this.logger.error(`Cache invalidation error for patterns ${patterns}:`, error);
    }
  }
   ...
   */
  getCacheMetrics(pattern?: string): CacheMetrics | Record<string, CacheMetrics> {
    if (pattern) {
  ...
  getCacheHealth(): {
    ...
    const allMetrics = Object.values(this.getCacheMetrics() as Record<string, CacheMetrics>);
    const totalRequests = allMetrics.reduce((sum, m) => sum + m.totalRequests, 0);
    const averageHitRatio = allMetrics.length > 0 
    ...
    if (averageHitRatio < 0.7) {
      issues.push(`Low cache hit ratio: ${(averageHitRatio * 100).toFixed(1)}%`);
      status = 'warning';
    }
  ...
  private generateCacheKey(pattern: string, params: Record<string, any>): string {
    ...
    const paramsHash = createHash('sha256').update(paramsString).digest('hex').substring(0, 16);

    return `stokcerdas:${pattern}:${paramsHash}`;
  }

  private getDefaultTTL(pattern: string, level: 'hot' | 'warm' | 'cold'): number {
    ...
    switch (level) {
      case 'hot': return 60; // 1 minute
      case 'warm': return 900; // 15 minutes
      case 'cold': return 3600; // 1 hour
      default: return 900;
    }
  }
  ...
  private async clearInMemoryCacheByPattern(pattern: string, tenantId?: string): Promise<void> {
  ...
  private async clearRedisCacheByPattern(pattern: string, tenantId?: string): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(`Failed to clear Redis cache by pattern ${searchPattern}:`, error);
    }
  }

  private updateMetrics(pattern: string, hit: boolean, responseTime: number): void {
