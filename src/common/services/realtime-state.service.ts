...
export class RealtimeStateService {
  ...
  async saveClientState(clientState: ClientState): Promise<void> {
    try {
      ...
      this.logger.debug(
        `💾 Saved client state for user ${clientState.userId} in tenant ${clientState.tenantId}`,
      );
    } catch (error) {
  ...
  async restoreClientState(
    ...
    try {
      ...
      if (clientState) {
        this.logger.debug(
          `🔄 Restored client state for user ${userId} in tenant ${tenantId}`,
        );
        return clientState;
      }
  ...
  async storePendingUpdate(
    ...
    try {
      ...
      this.logger.debug(
        `📦 Stored pending update for user ${userId} in tenant ${tenantId}`,
      );
    } catch (error) {
  ...
  async getPendingUpdates(tenantId: string, userId: string): Promise<any[]> {
    try {
      ...
      this.logger.debug(
        `📥 Retrieved ${updates.length} pending updates for user ${userId} in tenant ${tenantId}`,
      );
      return updates;
    } catch (error) {
  ...
  async updateClientHeartbeat(
    ...
    try {
      ...
      this.logger.debug(
        `💓 Updated heartbeat for user ${userId} in tenant ${tenantId}`,
      );
    } catch (error) {
  ...
  private canAutoMerge(localChanges: any, serverChanges: any): boolean {
    ...
    const overlappingFields = localFields.filter(field =>
    ...
    return overlappingFields.every(
      field =>
        JSON.stringify(localChanges[field]) ===
        JSON.stringify(serverChanges[field]),
    );
  }
